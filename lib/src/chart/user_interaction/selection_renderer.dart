...
class SelectionRenderer {
    ...
    if (chart is SfCartesianChart) {
      if (validIndex(pointIndex, seriesIndex, chart)) {
        ...
        viewportIndex = getVisibleDataPointIndex(
          pointIndex,
          SeriesHelper.getSeriesRendererDetails(seriesRender),
        );
      ...
      if (pointIndex! < chart.series.dataSource.length) {
        seriesRendererDetails.stateProperties.chartSeries.seriesPointSelection(
          pointIndex,
          chart.selectionGesture,
        );
      }
      ...
      if (seriesRendererDetails.seriesType.contains('area') == false &&
        ...
        final SegmentProperties currentSegmentProperties =
            SegmentHelper.getSegmentProperties(
              seriesRendererDetails.segments[selectedSegments[i]
                  .currentSegmentIndex],
            );
        final Paint fillPaint = getFillColor(
          true,
          currentSegmentProperties.segment,
        );
        ...
        final Paint strokePaint = getStrokeColor(
          true,
          currentSegmentProperties.segment,
        );
    ...
    for (int i = 0; i < unselectedSegments.length; i++) {
      if (unselectedSegments[i] is! ErrorBarSegment) {
        ...
        seriesRendererDetails = SeriesHelper.getSeriesRendererDetails(
          stateProperties
              .chartSeries
              .visibleSeriesRenderers[unselectedSegmentProperties.seriesIndex],
        );
        if (seriesRendererDetails.seriesType.contains('area') == false &&
            seriesRendererDetails.seriesType != 'fastline') {
          final SegmentProperties currentSegmentProperties =
              SegmentHelper.getSegmentProperties(
                seriesRendererDetails.segments[unselectedSegments[i]
                    .currentSegmentIndex],
              );
          final Paint fillPaint = getFillColor(
            false,
  ...
  void changeColorAndPopUnselectedSegments(
    ...
    while (unselectedSegments.isNotEmpty) {
      seriesRendererDetails = SeriesHelper.getSeriesRendererDetails(
        stateProperties
            .chartSeries
            .visibleSeriesRenderers[SegmentHelper.getSegmentProperties(
          unselectedSegments[k],
        ).seriesIndex],
      );
      if (seriesRendererDetails.seriesType.contains('area') == false &&
          seriesRendererDetails.seriesType != 'fastline') {
        if (unselectedSegments[k].currentSegmentIndex! <
          ...
          final SegmentProperties currentSegmentProperties =
              SegmentHelper.getSegmentProperties(
                seriesRendererDetails.segments[unselectedSegments[k]
                    .currentSegmentIndex],
              );
    ...
    while (selectedSegments.isNotEmpty) {
      ...
      if (seriesRendererDetails.seriesType.contains('area') == false &&
        ...
        if (selectedSegments[j].currentSegmentIndex! <
          ...
          final SegmentProperties currentSegmentProperties =
              SegmentHelper.getSegmentProperties(
                seriesRendererDetails.segments[selectedSegments[j]
                    .currentSegmentIndex],
              );
          final Paint fillPaint = getDefaultFillColor(
            null,
  ...
  bool isCartesianSelection(
    ...
    CartesianSeriesRenderer seriesAssign,
    int? pointIndex,
    int? seriesIndex,
  ) {
    chart = chartAssign;
      ...
      if (!(seriesRendererDetails.selectionBehavior.toggleSelection == false &&
        ...
        if (chart.enableMultiSelection == true) {
          if (selectedSegments.isNotEmpty) {
            for (
              int i =
                  stateProperties.chartSeries.visibleSeriesRenderers.length - 1;
              i >= 0;
              i--
            ) {
              final SeriesRendererDetails seriesRendererDetails =
            ...
            if (multiSelect) {
              for (int j = selectedSegments.length - 1; j >= 0; j--) {
                seriesRendererDetails = SeriesHelper.getSeriesRendererDetails(
                  stateProperties
                      .chartSeries
                      .visibleSeriesRenderers[selectedSegmentProperties!
                      .seriesIndex],
                );
                final SegmentProperties currentSegmentProperties =
                    SegmentHelper.getSegmentProperties(
        ...
        if (seriesRendererDetails.isSelectionEnable == true) {
          if (!isSamePointSelect) {
            seriesRendererDetails.seriesType == 'column' ||
                    ...
                    seriesRendererDetails.seriesType.contains(
                          'stackedcolumn',
                        ) ==
                        true ||
          ...
          if (selectedSegments.isNotEmpty) {
            ...
            if (multiSelect) {
              ...
              for (int j = selectedSegments.length - 1; j >= 0; j--) {
                ...
                currentSegmentProperties =
                    (seriesRendererDetails.seriesType.contains('area') ==
                                false &&
                            seriesRendererDetails.seriesType != 'fastline')
                        ? SegmentHelper.getSegmentProperties(
                          seriesRendererDetails.segments[selectedSegments[j]
  ...
  bool isLineIntersect(
    ...
    final int topPos = getPointDirection(
      segmentStartPoint,
      segmentEndPoint,
      touchStartPoint,
    );
    final int botPos = getPointDirection(
      segmentStartPoint,
      segmentEndPoint,
      touchEndPoint,
    );
    final int leftPos = getPointDirection(
      touchStartPoint,
      touchEndPoint,
      segmentStartPoint,
    );
    final int rightPos = getPointDirection(
      touchStartPoint,
      touchEndPoint,
      segmentEndPoint,
    );
  ...
  bool isSeriesContainsPoint(
    ...
    for (final CartesianChartPoint<dynamic> dataPoint in nearestDataPoints) {
      final int actualIndex = seriesRendererDetails.dataPoints.indexOf(
        dataPoint,
      );
      if (actualIndex >= 0) {
        dataPointIndex =
    ...
    if (dataPointIndex != null &&
      ...
      if (startSegmentProperties != null) {
        ...
        if (_isSegmentIntersect(
          startSegmentProperties.segment,
          position.dx,
          position.dy,
        )) {
          return true;
  ...
  int? getCartesianPointIndex(Offset position) {
    ...
    firstNearestDataPoints.add(
      seriesRendererDetails.dataPoints[previousDataPointIndex],
    );
    firstNearestDataPoints.add(
      seriesRendererDetails.dataPoints[nextDataPointIndex],
    );
    ...
    SelectionHelper.getRenderingDetails(
          seriesRendererDetails.selectionBehaviorRenderer,
        ).selectionRenderer!.cartesianPointIndex =
        nearestDataPointIndex;
  ...
  bool _isSegmentIntersect(
    ...
    if (currentSegment is HiloSegment ||
      ...
      y1 =
          currentSegment is StackedLineSegment ||
    ...
    final CartesianChartPoint<dynamic> leftPoint = CartesianChartPoint<dynamic>(
      touchX1 - 20,
      touchY1 - 20,
    );
    final CartesianChartPoint<dynamic> rightPoint =
        CartesianChartPoint<dynamic>(touchX1 + 20, touchY1 + 20);
    final CartesianChartPoint<dynamic> topPoint = CartesianChartPoint<dynamic>(
      touchX1 + 20,
      touchY1 - 20,
    );
