...
class SfCartesianChartState extends State<SfCartesianChart>
  ...
  void _initializeDefaultValues() {
    ...
    _stateProperties.renderingDetails.chartLegend = ChartLegend(
      _stateProperties,
    );
    ...
    _stateProperties.zoomPanBehaviorRenderer = ZoomPanBehaviorRenderer(
      _stateProperties,
    );
    _stateProperties.trackballBehaviorRenderer = TrackballBehaviorRenderer(
      _stateProperties,
    );
    _stateProperties.crosshairBehaviorRenderer = CrosshairBehaviorRenderer(
      _stateProperties,
    );
    _stateProperties
        .renderingDetails
        .tooltipBehaviorRenderer = TooltipBehaviorRenderer(_stateProperties);
    _stateProperties.renderingDetails.legendRenderer = LegendRenderer(
      widget.legend,
    );
    _stateProperties
        .trackballMarkerSettingsRenderer = TrackballMarkerSettingsRenderer(
  ...
  void didUpdateWidget(SfCartesianChart oldWidget) {
    ...
    if (_stateProperties.isNeedUpdate) {
      ...
      _stateProperties.oldAxisRenderers =
          <ChartAxisRenderer>[]
            //ignore: prefer_spread_collections
  ...
  Future<dart_ui.Image> toImage({double pixelRatio = 1.0}) async {
    ...
    final dart_ui.Image image = await boundary.toImage(
  ...
  void _getOldSeriesKeys(List<CartesianSeriesRenderer> oldSeriesRenderers) {
    ...
    for (int i = 0; i < oldSeriesRenderers.length; i++) {
      _stateProperties.oldSeriesKeys.add(
        SeriesHelper.getSeriesRendererDetails(oldSeriesRenderers[i]).series.key,
      );
    }
  }
      ...
      for (int i = 0; i < seriesLength; i++) {
        ...
        if (oldWidget != null) {
          if (oldWidgetOldSeriesRenderers!.isNotEmpty) {
            ...
            index =
                i < oldWidgetOldSeriesRenderers.length &&
                        isSameSeries(
                          SeriesHelper.getSeriesRendererDetails(
          ...
          if (oldWidgetSeriesRenderers!.isNotEmpty) {
            ...
            oldSeriesIndex =
                i < oldWidgetSeriesRenderers.length &&
                        isSameSeries(
                          SeriesHelper.getSeriesRendererDetails(
                            oldWidgetSeriesRenderers[i],
                          ).series,
        ...
        if (index != null &&
          ...
          if (seriesRenderer is XyDataSeriesRenderer) {
            seriesRendererDetails.animationController = AnimationController(
              vsync: this,
            )..addListener(seriesRendererDetails.repaintSeriesElement);
            _stateProperties.controllerList[seriesRendererDetails
                    .animationController] =
          ...
          seriesRendererDetails.controller ??= ChartSeriesController(
  ...
  int? _getExistingSeriesIndex(
    CartesianSeries<dynamic, dynamic> currentSeries,
  ...
  Widget _renderTitle() {
    ...
    if (_stateProperties.chart.title.text != null &&
      ...
      final TextStyle titleStyle = getTextStyle(
        ...
        fontColor:
            _stateProperties.chart.title.textStyle.color ??
      ...
      titleWidget = Container(
        ...
        child: Container(
          decoration: BoxDecoration(
            color:
                _stateProperties.chart.title.backgroundColor ??
                _stateProperties
                    .renderingDetails
                    .chartTheme
            ...
            border: Border.all(
              color:
                  _stateProperties.chart.title.borderWidth == 0
                      ? Colors.transparent
                      : _stateProperties
                          .chart
                          .title
          ...
          child: Text(
            _stateProperties.chart.title.text,
      ...
      child: LayoutBuilder(
        builder: (BuildContext context, BoxConstraints constraints) {
          ...
          if (legendTemplates.isNotEmpty &&
            ...
            element = getElements(
              _stateProperties,
              _stateProperties.containerArea,
              constraints,
            );
          }
          return element!;
  ...
  void _findVisibleSeries(BuildContext context) {
    ...
    for (int i = 0; i < seriesRenderers.length; i++) {
      ...
      final CartesianSeries<dynamic, dynamic> cartesianSeries =
          seriesRendererDetails.series;
      seriesRendererDetails.markerSettingsRenderer = MarkerSettingsRenderer(
      ...
      if (cartesianSeries.trendlines != null) {
        ...
        for (final TrendlineRenderer trendlineRenderer
          ...
          trendlineRenderer.name =
              trendlineRenderer.name![0].toUpperCase() +
          ...
          if (trendlineTypes[trendline.type.index] == 1 &&
              trendlineRenderer.name![trendlineRenderer.name!.length - 1] ==
                  '0') {
            trendlineRenderer.name = trendlineRenderer.name!.substring(
              0,
              trendlineRenderer.name!.length - 2,
            );
          }
        }
      }
      ...
      if (i == 0 ||
        ...
        final CartesianSeriesRenderer? cSeriesRenderer =
            _stateProperties
                        .chartSeries
                        .visibleSeriesRenderers[visibleSeriesRenderers.length -
                        1]
        ...
        if (cSeriesDetails.series != null &&
          ...
          for (int j = 0; j < cSeriesDetails.series.trendlines!.length; j++) {
            ...
            trendlineRenderer.visible =
                _checkWithTrendlineLegendToggleState(
                  visibleSeriesRenderers.length - 1,
    ...
    if (_stateProperties.chart.indicators.isNotEmpty) {
      ...
      for (int i = 0; i < _stateProperties.chart.indicators.length; i++) {
        technicalIndicatorRenderer = TechnicalIndicatorsRenderer(
          _stateProperties.chart.indicators[i],
          _stateProperties,
        );
        _stateProperties.technicalIndicatorRenderer.add(
  ...
  void _validateStateMaintenance(
    ...
    if (stateProperties.renderingDetails.oldDeviceOrientation !=
      ...
      if (crosshairRenderingDetails.position != null &&
        ...
        crosshairOffset = calculatePointToPixel(
          point,
          stateProperties.seriesRenderers.first,
        );
...
class _ContainerAreaState extends State<ContainerArea> {
  ...
  Widget build(BuildContext context) {
    ...
    final bool requireInvertedAxis =
        widget._stateProperties.seriesRenderers.isNotEmpty
            ? (chart.isTransposed ^
                getSeriesType(
                  widget._stateProperties.seriesRenderers[0],
                ).toLowerCase().contains('bar'))
            : chart.isTransposed;
            ...
            child: Listener(
              onPointerDown: (PointerDownEvent event) {
                if (widget._stateProperties.chartState.mounted) {
                  ...
                  if (chart.onChartTouchInteractionDown != null) {
                    ...
                    touchArgs.position = renderBox.globalToLocal(
                      event.position,
                    );
              ...
              child: GestureDetector(
                onTapDown: (TapDownDetails details) {
                  if (widget._stateProperties.chartState.mounted) {
                    final Offset position = renderBox.globalToLocal(
                  ...
                  if (widget._stateProperties.chartState.mounted &&
                    ...
                    if (selectionSeriesDetails.visible ?? false) {
                      ...
                      if (selectionSeriesDetails.isSelectionEnable == true &&
                        ...
                        selectionDetails
                            ?.selectionRenderer
                            ?.seriesRendererDetails = selectionSeriesDetails;
  ...
  void _calculateContainerSize(BoxConstraints constraints) {
    final double width = constraints.maxWidth;
    final double height = constraints.maxHeight;
    widget._stateProperties.renderingDetails.chartContainerRect = Rect.fromLTWH(
      0,
      0,
      width,
      height,
              ...
              widget._stateProperties.isLoadMoreIndicator
                  ? Center(
                    child: widget
                        ...
                        .loadMoreIndicatorBuilder!(
                      context,
                      widget._stateProperties.swipeDirection,
                    ),
                  )
      ...
      if (_seriesRenderer != null &&
        ...
        for (
          ...
          if (trendline.animationDuration > 0 &&
            ...
            trendlineRenderer.animationController.duration = Duration(
              milliseconds: totalAnimationDuration,
            );
            ...
            minSeriesInterval =
                ...
                minSeriesInterval;
            trendlineAnimations['$i-$j'] = Tween<double>(
              begin: 0.0,
              end: 1.0,
            ).animate(
              CurvedAnimation(
        ...
        RepaintBoundary(
          child: CustomPaint(
            painter: TrendlinePainter(
              ...
              notifier: widget._stateProperties.repaintNotifiers['trendline']!,
            ),
          ),
        ),
      );
  ...
  void _renderTemplates() {
    ...
    if (chart.annotations != null && chart.annotations!.isNotEmpty) {
      for (int i = 0; i < chart.annotations!.length; i++) {
        ...
        final ChartLocation location = getAnnotationLocation(
          annotation,
          widget._stateProperties,
        );
        final ChartTemplateInfo chartTemplateInfo = ChartTemplateInfo(
          ...
          clipRect:
              annotation.region == AnnotationRegion.chart
        ...
        widget._stateProperties.renderingDetails.templates.add(
          chartTemplateInfo,
        );
      }
    }
  ...
  void _renderDataLabelTemplates() {
    ...
    for (
      ...
      num padding;
      final List<CartesianChartPoint<dynamic>> dataPoints = getSampledData(
        seriesRendererDetails,
      );
    ...
    for (
      ...
      if (seriesRendererDetails.isIndicator == true) {
        ...
        if (_seriesRenderer is XyDataSeriesRenderer) {
          seriesRendererDetails.animationController = AnimationController(
            vsync: widget._stateProperties.chartState,
          )..addListener(seriesRendererDetails.repaintSeriesElement);
          widget._stateProperties.controllerList[seriesRendererDetails
                  .animationController] =
              seriesRendererDetails.repaintSeriesElement;
          seriesRendererDetails.animationController.addStatusListener(
            seriesRendererDetails.animationStatusListener,
      ...
      if (_seriesRenderer != null && seriesRendererDetails.visible! == true) {
        ...
        if (selectionBehaviorRenderer != null) {
          final sync_selection.SelectionDetails selectionDetails =
          ...
          if (widget._stateProperties.isRangeSelectionSlider == false &&
                  ...
                  (!_renderingDetails.initialRender! &&
                      (totalSelectedSegment != 0 &&
                          (totalSelectedSegment <
                              SeriesHelper.getSeriesRendererDetails(
                                widget._stateProperties.seriesRenderers[i],
                              ).dataPoints.length))))) {
            ...
            for (int j = 0; j < segmentLength; j++) {
              ...
              SegmentHelper.setSegmentProperties(
                segment,
                SegmentProperties(widget._stateProperties, segment),
              );
              final SegmentProperties segmentProperties =
              ...
              ((_series.initialSelectedDataIndexes!.contains(
                  ...
                  ? SelectionHelper.getRenderingDetails(
                    selectionBehaviorRenderer,
                  ).selectionRenderer!.selectedSegments.add(segment)
                  : SelectionHelper.getRenderingDetails(
                    selectionBehaviorRenderer,
                  ).selectionRenderer!.unselectedSegments!.add(segment);
            }
            seriesRendererDetails.oldSelectedIndexes =
                <int>[]
                  //ignore: prefer_spread_collections
        ...
        if (seriesRendererDetails.animationController != null &&
          ...
          seriesRendererDetails.seriesAnimation = Tween<double>(
            begin: 1,
            end: 1.0,
          ).animate(
            CurvedAnimation(
          ...
          seriesRendererDetails.seriesElementAnimation = Tween<double>(
            begin: 1,
            end: 1.0,
          ).animate(
            CurvedAnimation(
        ...
        child: RepaintBoundary(
          child: CustomPaint(
            painter: ZoomRectPainter(
              stateProperties: widget._stateProperties,
              notifier: widget._stateProperties.repaintNotifiers['zoom'],
            ),
          ),
        ),
      ),
  ...
  CartesianSeriesRenderer? _findSeries(Offset position) {
    ...
    for (
      int i =
          widget._stateProperties.chartSeries.visibleSeriesRenderers.length - 1;
      i >= 0;
      i--
    ) {
      seriesRenderer =
      ...
      if (seriesType == 'column' ||
        ...
        for (
          ...
          isSelect =
              seriesRendererDetails.isSelectionEnable == true &&
              seriesRendererDetails.visible != null &&
              seriesRendererDetails.visible! == true &&
              ...
              (SelectionHelper.getRenderingDetails(
                    selectionBehaviorRenderer!,
                  ).selectionRenderer!.isSeriesContainsPoint(
                    SeriesHelper.getSeriesRendererDetails(
                      widget
                          ._stateProperties
                          .chartSeries
  ...
  void _performPointerDown(PointerDownEvent event) {
          ...
          final ChartAxisRendererDetails axisDetails =
              AxisHelper.getAxisRendererDetails(
                widget
                    ._stateProperties
                    .chartAxis
                    .axisRenderersCollection[axisIndex],
              );

          if (chart.onZoomStart != null) {
            zoomStartArgs = bindZoomEvent(
              chart,
              axisDetails,
              chart.onZoomStart!,
            );
    ...
    final CartesianSeriesRenderer? cartesianSeriesRenderer = _findSeries(
      position,
    );
    // ignore: unnecessary_null_comparison
    if (chart.trackballBehavior != null &&
      ...
      if (chart.trackballBehavior.builder != null) {
        ...
        widget._stateProperties.trackballBehaviorRenderer.onTouchDown(
          position.dx,
          position.dy,
        );
      }
    ...
    if (chart.crosshairBehavior != null &&
        ...
        SeriesHelper.getSeriesRendererDetails(cartesianSeriesRenderer).series
            is! ErrorBarSeries) {
      widget._stateProperties.crosshairBehaviorRenderer.onTouchDown(
        position.dx,
        position.dy,
      );
    }
    ...
    if ((chart.trackballBehavior != null &&
        ...
        (chart.zoomPanBehavior != null &&
            ((chart.zoomPanBehavior.enableDoubleTapZooming ||
                ...
                !chart.trackballBehavior.shouldAlwaysShow))) {
      widget._stateProperties.trackballBehaviorRenderer.onTouchUp(
        position.dx,
        position.dy,
      );
    ...
    if ((chart.crosshairBehavior != null &&
        ...
        (chart.zoomPanBehavior != null &&
            ((chart.zoomPanBehavior.enableDoubleTapZooming ||
                    ...
                    chart.zoomPanBehavior.enableSelectionZooming) &&
                !chart.crosshairBehavior.shouldAlwaysShow))) {
      widget._stateProperties.crosshairBehaviorRenderer.onTouchUp(
        position.dx,
        position.dy,
      );
  ...
  void _calculatePinchZoomingArgs() {
    ...
    if (resetFlag) {
      for (
        ...
        final ChartAxisRendererDetails axisDetails =
            AxisHelper.getAxisRendererDetails(
              widget._stateProperties.chartAxis.axisRenderersCollection[index],
            );
  ...
  void _performLongPressMoveUpdate(LongPressMoveUpdateDetails details) {
    ...
    if (chart.trackballBehavior != null &&
      ...
      if (chart.trackballBehavior.activationMode == ActivationMode.singleTap) {
        chart.trackballBehavior.builder != null
            ? trackballRenderingDetails.showTemplateTrackball(position)
            : widget._stateProperties.trackballBehaviorRenderer.onTouchMove(
              position.dx,
              position.dy,
            );
      }
    ...
    if (chart.crosshairBehavior != null &&
      ...
      if (chart.crosshairBehavior.activationMode == ActivationMode.singleTap) {
        widget._stateProperties.crosshairBehaviorRenderer.onTouchMove(
          position.dx,
          position.dy,
        );
        // ignore: unnecessary_null_comparison
  ...
  void _performLongPressEnd() {
    ...
    if (zoomingBehaviorDetails.isPinching != true) {
      ...
      if (chart.zoomPanBehavior.enableSelectionZooming &&
        ...
        zoomingBehaviorDetails.doSelectionZooming(
          zoomingBehaviorDetails.zoomingRect,
        );
  ...
  void _performPanDown(DragDownDetails details) {
    widget._stateProperties.startOffset = renderBox.globalToLocal(
      details.globalPosition,
    );
    final ZoomingBehaviorDetails zoomingBehaviorDetails =
        ZoomPanBehaviorHelper.getRenderingDetails(
  ...
  void _performDoubleTap() {
    if (_tapDownDetails != null) {
      ...
      if (chart.trackballBehavior != null &&
        ...
        chart.trackballBehavior.builder != null
            ? trackballRenderingDetails.showTemplateTrackball(position)
            : widget._stateProperties.trackballBehaviorRenderer.onDoubleTap(
              position.dx,
              position.dy,
            );
      ...
      if (chart.crosshairBehavior != null &&
          ...
          SeriesHelper.getSeriesRendererDetails(cartesianSeriesRenderer).series
              is! ErrorBarSeries &&
          chart.crosshairBehavior.activationMode == ActivationMode.doubleTap) {
        widget._stateProperties.crosshairBehaviorRenderer.onDoubleTap(
          position.dx,
          position.dy,
        );
        widget._stateProperties.enableDoubleTap = true;
  ...
  void _performPanUpdate(DragUpdateDetails details) {
    if (widget._stateProperties.chartState.mounted) {
      Offset? position;
      widget._stateProperties.currentPosition = renderBox.globalToLocal(
        details.globalPosition,
      );
      final ZoomingBehaviorDetails zoomingBehaviorDetails =
          ZoomPanBehaviorHelper.getRenderingDetails(
      ...
      if (zoomingBehaviorDetails.isPinching != true) {
        ...
        if ((zoomingBehaviorDetails.isPanning ?? false) &&
            ...
            zoomingBehaviorDetails.previousMovedPosition != null &&
            !widget._stateProperties.isLoadMoreIndicator) {
          widget._stateProperties.zoomPanBehaviorRenderer.onPan(
            position.dx,
            position.dy,
          );
        }
  ...
  void _performPanEnd(DragEndDetails details) {
    if (widget._stateProperties.chartState.mounted) {
      ...
      if (chart.onPlotAreaSwipe != null &&
          ...
          widget._stateProperties.chartAxis.axisClipRect.contains(
        ...
        if (widget._stateProperties.requireInvertedAxis &&
          ...
          ///vertical
          swipeDirection =
              widget._stateProperties.pointerDeviceKind ==
  ...
  void _performMouseHover(PointerEvent event) {
    if (widget._stateProperties.chartState.mounted) {
      ...
      if (chart.trackballBehavior.enable &&
        ...
        chart.trackballBehavior.builder != null
            ? trackballRenderingDetails.showTemplateTrackball(position)
            : widget._stateProperties.trackballBehaviorRenderer.onEnter(
              position.dx,
              position.dy,
            );
      }
      if (chart.crosshairBehavior.enable &&
          ...
          SeriesHelper.getSeriesRendererDetails(cartesianSeriesRenderer).series
              is! ErrorBarSeries &&
          chart.crosshairBehavior.activationMode == ActivationMode.singleTap) {
        widget._stateProperties.crosshairBehaviorRenderer.onEnter(
          position.dx,
          position.dy,
        );
      }
  ...
  void _performMouseExit(PointerEvent event) {
    if (widget._stateProperties.chartState.mounted) {
      ...
      if (chart.tooltipBehavior.enable ||
          shouldShowAxisTooltip(widget._stateProperties)) {
        widget._stateProperties.renderingDetails.tooltipBehaviorRenderer.onExit(
          position.dx,
          position.dy,
        );
      }
      if (chart.crosshairBehavior.enable) {
        widget._stateProperties.crosshairBehaviorRenderer.onExit(
          position.dx,
          position.dy,
        );
      }
      if (chart.trackballBehavior.enable) {
        widget._stateProperties.trackballBehaviorRenderer.onExit(
          position.dx,
          position.dy,
        );
      }
  ...
  void _bindInteractionWidgets(
    ...
    final ZoomRectPainter zoomRectPainter = ZoomRectPainter(
      stateProperties: widget._stateProperties,
    );
    final ZoomingBehaviorDetails zoomingBehaviorDetails =
        ZoomPanBehaviorHelper.getRenderingDetails(
    ...
    if (chart.trackballBehavior != null && chart.trackballBehavior.enable) {
      if (chart.trackballBehavior.builder != null) {
        ...
        userInteractionWidgets.add(
          trackballRenderingDetails.trackballTemplate!,
        );
      } else {
        trackballPainter = TrackballPainter(
          stateProperties: widget._stateProperties,
          valueNotifier: widget._stateProperties.repaintNotifiers['trackball']!,
        );
    ...
    if (chart.crosshairBehavior != null && chart.crosshairBehavior.enable) {
      crosshairPainter = CrosshairPainter(
        stateProperties: widget._stateProperties,
        valueNotifier: widget._stateProperties.repaintNotifiers['crosshair']!,
      );
      CrosshairHelper.getRenderingDetails(
            widget._stateProperties.crosshairBehaviorRenderer,
          ).crosshairPainter =
    ...
    if (chart.tooltipBehavior.enable ||
      ...
      tooltipRenderingDetails.chartTooltip = SfTooltip(
        ...
        onTooltipRender:
            chart.onTooltipRender != null
                ? tooltipRenderingDetails.tooltipRenderingEvent
                : null,
      );
      _renderingDetails.chartWidgets!.add(
        tooltipRenderingDetails.chartTooltip!,
      );
    }
    ...
    switch (seriesRendererDetails.seriesType) {
      case 'line':
        customPainter = LineChartPainter(
          ...
          isRepaint:
              widget._stateProperties.zoomedState != null
