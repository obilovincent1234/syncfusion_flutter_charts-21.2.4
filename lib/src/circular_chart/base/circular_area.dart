...
class CircularArea extends StatelessWidget {
        ...
        return Container(
          child: MouseRegion(
            ...
            child: Listener(
              ...
              onPointerMove:
                  (PointerMoveEvent event) => _performPointerMove(event),
  ...
  void _onPointerDown(PointerDownEvent event) {
    ...
    stateProperties.renderingDetails.tapPosition = renderBox.globalToLocal(
      event.position,
    );
    ...
    if (stateProperties.renderingDetails.tapPosition != null) {
      ...
      if (chart.tooltipBehavior.enable &&
        ...
        if (chart.tooltipBehavior.builder != null) {
          ...
          stateProperties.renderingDetails.tooltipBehaviorRenderer.onTouchUp(
    ...
    if (stateProperties.renderingDetails.tapPosition != null) {
      if (chart.tooltipBehavior.enable &&
        ...
        if (chart.tooltipBehavior.builder != null) {
          ...
          stateProperties.renderingDetails.tooltipBehaviorRenderer.onEnter(
            position.dx.toDouble(),
            position.dy.toDouble(),
          );
        }
  ...
  void showCircularTooltipTemplate([int? seriesIndex, int? pointIndex]) {
    ...
    if (point.isVisible) {
      final Offset? location = degreeToPoint(
        point.midAngle!,
  ...
  void _calculateContainerSize(BoxConstraints constraints) {
    ...
    stateProperties.renderingDetails.chartContainerRect = Rect.fromLTWH(
      0,
      0,
      width.toDouble(),
      height.toDouble(),
        ...
        for (int i = 0; i < seriesRenderer.renderPoints!.length; i++) {
          ...
          if (point.isVisible) {
            ...
            if (series.dataLabelSettings.labelPosition ==
              ...
              labelLocation = degreeToPoint(
                point.midAngle!,
                point.outerRadius! + connectorLength,
                point.center!,
              );
            ...
            stateProperties.renderingDetails.templates.add(
              ChartTemplateInfo(
                key: GlobalKey(),
      ...
      for (int i = 0; i < chart.annotations!.length; i++) {
        ...
        if (annotation.widget != null) {
          radius =
              percentToValue(
                annotation.radius,
                stateProperties.chartSeries.size / 2,
              )!.toDouble();
          point = degreeToPoint(
            annotation.angle,
            radius,
            stateProperties.centerLocation,
          );
          annotationHeight =
              percentToValue(
                annotation.height,
          ...
          templateInfo = ChartTemplateInfo(
            ...
            widget:
                annotationHeight > 0 && annotationWidth > 0
                    ? SizedBox(
  ...
  void _renderTemplates() {
    if (stateProperties.renderingDetails.templates.isNotEmpty) {
      ...
      stateProperties.renderingDetails.chartWidgets!.add(
        stateProperties.renderingDetails.chartTemplate!,
      );
    }
  }
  ...
  void _bindTooltipWidgets(BoxConstraints constraints) {
    ...
    if (chart.tooltipBehavior.enable ||
      ...
      final Widget uiWidget = IgnorePointer(
        child: Stack(children: <Widget>[tooltipRenderingDetails.chartTooltip!]),
      );
  ...
  void _bindSeriesWidgets(BuildContext context) {
    ...
    for (
      ...
      if (series.initialSelectedDataIndexes.isNotEmpty) {
        for (
          ...
          stateProperties.renderingDetails.selectionData.add(
            series.initialSelectedDataIndexes[index],
          );
        }
      }
      stateProperties.renderingDetails.animateCompleted = false;
      if (series.animationDuration > 0 &&
        ...
        stateProperties.renderingDetails.chartElementAnimation = Tween<double>(
          begin: 0.0,
          end: 1.0,
        ).animate(
          CurvedAnimation(
      ...
      stateProperties.renderingDetails.chartWidgets!.add(
        RepaintBoundary(child: CustomPaint(painter: seriesPainter)),
      );
      ...
      stateProperties.renderingDetails.chartWidgets!.add(
        stateProperties.renderDataLabel!,
      );
    }
  }
}
...
void _showTrimmedDataLabelTooltip(
  ...
  for (int i = 0; i < seriesRenderer.renderPoints!.length; i++) {
    if (seriesRenderer.series.dataLabelSettings.isVisible &&
        seriesRenderer.renderPoints![i].labelRect.contains(
