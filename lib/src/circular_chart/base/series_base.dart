...
class CircularSeriesBase {
        ...
        if (seriesRenderer.series.sortingOrder == SortingOrder.ascending) {
          return (firstPoint.sortValue == null)
              ...
              : (secondPoint.sortValue == null
                      ...
                      : (firstPoint.sortValue is String
                          ? firstPoint.sortValue.toLowerCase().compareTo(
                            secondPoint.sortValue.toLowerCase(),
                          )
                          : firstPoint.sortValue.compareTo(
          ...
          return (firstPoint.sortValue == null)
              ...
              : (secondPoint.sortValue == null
                      ? -1
                      : (firstPoint.sortValue! is String
                          ? secondPoint.sortValue!.toLowerCase().compareTo(
                            firstPoint.sortValue!.toLowerCase(),
                          )
                          : secondPoint.sortValue!.compareTo(
  ...
  void _findingGroupPoints() {
    ...
    if (sumOfGroup > 0) {
      seriesRenderer.renderPoints!.add(
        ChartPoint<dynamic>('Others', sumOfGroup),
      );
      seriesRenderer
          .renderPoints![seriesRenderer.renderPoints!.length - 1]
  ...
  void _setPointStyle(CircularSeriesRendererExtension seriesRenderer) {
    ...
    for (final ChartPoint<dynamic> point in seriesRenderer.renderPoints!) {
      ...
      point.fill =
          point.isEmpty && empty.color != null
              ? empty.color
              : point.pointColor ?? palette[i % palette.length];
      point.color = point.fill;
      // ignore: unnecessary_null_comparison
      point.strokeColor =
          point.isEmpty && empty.borderColor != null
              ? empty.borderColor
              : currentSeries.borderColor;
      // ignore: unnecessary_null_comparison
      point.strokeWidth =
          point.isEmpty && empty.borderWidth != null
              ? empty.borderWidth
    ...
    for (final ChartPoint<dynamic> point in seriesRenderer.renderPoints!) {
      if (point.isVisible) {
        if (radialSeries) {
          ...
          point.outerRadius =
              seriesRenderer.renderPoints![point.index].outerRadius =
                  ringSize! < gap!
                      ? 0
                      : seriesRenderer.renderPoints![point.index].innerRadius! +
                          ringSize -
                          gap -
                          (series.trackBorderWidth / 2) /
          ...
          point.center =
              _needExplode(pointIndex, currentSeries)
                  ? _findExplodeCenter(
                    point.midAngle!,
                    seriesRenderer,
                    point.outerRadius!,
  ...
  bool _needExplode(int pointIndex, CircularSeries<dynamic, dynamic> series) {
    ...
    if (series.explode) {
      if (stateProperties.renderingDetails.initialRender!) {
        ...
        if (!stateProperties.renderingDetails.explodedPoints.contains(
              pointIndex,
            ) &&
          ...
          if (pointIndex == series.explodeIndex || series.explodeAll) {
            ...
            stateProperties.renderingDetails.explodedPoints.removeAt(
              pointIndex,
            );
          }
        }
    ...
    if (seriesRenderer.isSelectionEnable && mode == chart.selectionGesture) {
      if (stateProperties.renderingDetails.selectionData.isNotEmpty) {
        if (!chart.enableMultiSelection &&
          ...
          if (stateProperties.renderingDetails.selectionData.contains(
            pointIndex,
          )) {
  ...
  void seriesPointExplosion(Region? pointRegion) {
    ...
    final CircularSeriesRendererExtension seriesRenderer =
        stateProperties.chartSeries.visibleSeriesRenderers[pointRegion!
            .seriesIndex];
    ...
    if (seriesRenderer.series.explode) {
      if (stateProperties.renderingDetails.explodedPoints.isNotEmpty) {
        if (stateProperties.renderingDetails.explodedPoints.length == 1 &&
            stateProperties.renderingDetails.explodedPoints.contains(
      ...
      if (!existExplodedRegion) {
        ...
        stateProperties.renderingDetails.explodedPoints.add(
          pointRegion.pointIndex,
        );
        stateProperties.renderingDetails.seriesRepaintNotifier.value++;
