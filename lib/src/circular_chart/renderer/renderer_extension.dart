...
class CircularSeriesRendererExtension implements CircularSeriesRenderer {
        ...
        for (
          ...
          if (currentPointIndex == selectionIndex) {
            pointStyle = StyleOptions(
              fill:
                  seriesRenderer.selectionArgs != null
  ...
  num? circularRenderPoint(
    ...
    if (isDynamicUpdate) {
      if (!oldPoint.isVisible && point.isVisible) {
        final num val =
            point.startAngle == seriesRenderer.segmentRenderingValues['start']!
                ? seriesRenderer.segmentRenderingValues['start']!
                : oldPointList![getVisiblePointIndex(
                      oldPointList,
                      'before',
                      pointIndex,
                    )!]!
        ...
        pointStartAngle =
            (point.startAngle! > oldStartAngle)
                ? oldStartAngle +
  ...
  void drawDataPoints(
    ...
    final StyleOptions? style = selectPoint(
      pointIndex,
      seriesRenderer,
      chart,
      point,
    ...
    final Color? strokeColor =
        style != null && style.strokeColor != null
            ? style.strokeColor
    ...
    final num? strokeWidth =
        style != null && style.strokeWidth != null
            ? style.strokeWidth
    ...
    assert(
      seriesRenderer.series.opacity >= 0,
      'The opacity value will not accept negative numbers.',
    );
    assert(
      seriesRenderer.series.opacity <= 1,
      'The opacity value must be less than 1.',
    );
    final double? opacity =
        style != null && style.opacity != null
            ? style.opacity
        ...
        if (point.isVisible) {
          ...
          if (stopsList.isEmpty) {
            ...
            if (stopsList.length == 1) {
              stopsList.add(
                (point.y! / segmentRenderingValues['sumOfPoints']! +
                        stopsList.last) +
                    initStops / 1.5,
              );
            } else {
...
class RadialBarSeriesRendererExtension extends RadialBarSeriesRenderer
  ...
  void _renderRadialPoints(
    ...
    final StyleOptions? style = seriesRenderer.selectPoint(
      i,
      seriesRenderer,
      chart,
      point,
  ...
  void _applyShadow(
    ...
    if (pointEndAngle != null && actualDegree > 360) {
      ...
      final Offset? midPoint = degreeToPoint(
        pointEndAngle,
        (currentInnerRadius + outerRadius) / 2,
        center!,
      );
      if (actualRadius > 0) {
        ...
        if (series.cornerStyle == CornerStyle.endCurve ||
          ...
          final Offset? endPoint = degreeToPoint(
            pointEndAngle,
            currentInnerRadius + (currentInnerRadius * 0.025),
            center!,
          );

          final Offset? overFilledStartPoint = degreeToPoint(
            pointEndAngle - 2,
            outerRadius,
            center!,
          );
          final Offset? overFilledEndPoint = degreeToPoint(
            pointEndAngle - 2,
            currentInnerRadius,
            center!,
          );
